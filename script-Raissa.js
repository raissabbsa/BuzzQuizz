...
function infoBasicas() {
  ...
  const tituloOk =
    tituloQuizz.value.length >= 20 && tituloQuizz.value.length <= 65;
...
function criandoQuizz() {
  ...
  for (let i = 0; i < qntdPerguntas.value; i++) {
    tela32.innerHTML += `<div data-identifier="question-form" class="perg pergunta${
      i + 1
    }">
...
function criaPerguntasQuizz() {
  ...
  for (let i = 0; i < maisPerguntas.length; i++) {
    ...
    urlImagemPerguntaIn = maisPerguntas[i].querySelectorAll(
      ".urlImagemPerguntaIn"
    );
    let cont = 0;
...
function enviaQuizz() {
  ...
  for (let i = 0; i < qntdPerguntas.value; i++) {
    ...
    const urlImagemPerguntaIn = maisPerguntas[i].querySelectorAll(
      ".urlImagemPerguntaIn"
    );
